...
const registerUser = (req,res)=>{
    ...
    let form = new driverModel(req.body)
    form.save((err)=>{
        if(err){
...
const authenticateUser = (req,res)=>{
    ...
    console.log(req.body)
    driverModel.findOne({email:req.body.email},(err,user)=>{
        if(err){
...
const dashboard=(req,res)=>{
    ...
    driverModel.findOne({email:email},(err, result)=>{
        if(err){
            console.log(err);
        }
        else{
...
const uploadFile=(req,res)=>{
    ...
    if(err){
        ...
        let img =result.url
        driverModel.findOneAndUpdate({email:req.body.currentUser},{image:img},(err,result)=>{
            if (err){
                console.log(err);
